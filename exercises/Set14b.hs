...
balance db account = foldr (+) 0 . fmap head <$> query db balanceQuery [account]
...
parseCommand' "deposit" [account, amount] = Deposit account <$> parseInt amount
parseCommand' "withdraw" [account, amount] = Withdraw account <$> parseInt amount
parseCommand' "balance" [account] = Just (Balance account)
parseCommand' _ _ = Nothing

parseCommand :: [T.Text] -> Maybe Command
parseCommand (command : args) = parseCommand' (T.unpack command) args
...
perform db (Just (Deposit account amount)) =
  T.pack "OK" <$ deposit db account amount
perform db (Just (Withdraw account amount)) =
  T.pack "OK" <$ deposit db account (-amount)
perform db (Just (Balance account)) = T.pack . show <$> balance db account
...
server db request respond = do
  response <- perform db . parseCommand . pathInfo $ request
