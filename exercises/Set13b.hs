...
visit maze place = do
  visited <- get
  when (not . elem place $ visited) (do
    put (place : visited)
    forM_ (lookup place maze) . mapM_ . visit $ maze)
...
instance Functor SL where
  -- implement fmap
  fmap f (SL sl) = SL (transform . sl)
    where transform (a, s, l) = (f a, s, l)
