...
mapMaybe2 _f Nothing _        = Nothing
mapMaybe2 _f _       Nothing  = Nothing
...
joinToLength len strs = filter (\str -> length str == len) [a ++ b | a <- strs, b <- strs]
...
interpreter' x y [] = []
interpreter' x y ("up":commands) = interpreter' x (y+1) commands
interpreter' x y ("down":commands) = interpreter' x (y-1) commands
interpreter' x y ("left":commands) = interpreter' (x-1) y commands
interpreter' x y ("right":commands) = interpreter' (x+1) y commands
interpreter' x y ("printX":commands) = show x : interpreter' x y commands
interpreter' x y ("printY":commands) = show y : interpreter' x y commands
