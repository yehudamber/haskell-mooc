...
inputInOutput input output = forAll (elements input)
  (\x -> elem x . map fst $ output)
...
outputInInput input output = forAll (elements output)
  (\(x, n) -> length (filter (== x) input) === n)
...
genList = do
  len <- chooseInt (3, 5)
  list <- vectorOf len . chooseInt $ (0, 10)
  return (sort list)
...
instance Arbitrary Arg where
  arbitrary = oneof
    [fmap Number (chooseInt (0, 10)), fmap Variable (elements "abcxyz")]
