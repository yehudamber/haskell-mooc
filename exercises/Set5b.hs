...
treeMax Empty = 0
treeMax (Node x l r) = max x $ max (treeMax l) (treeMax r)
...
isOrdered (Node x l r) = isOrdered l && isOrdered r && allValues (< x) l && allValues (> x) r
...
walk (s:steps) (Node _ l r) = walk steps (if s == StepL then l else r)
...
search val (Node x l r) = if x == val
  then Just []
  else maybe (fmap (StepR:) $ search val r) (Just . (StepL:)) $ search val l
