...
prettyPrint n queens = concat . map row $ [1 .. n]
    where row i = map cell [1 .. n] ++ "\n"
            where cell j = if elem (i, j) queens then 'Q' else '.'
...
danger' a b = sameRow a b || sameCol a b || sameDiag a b || sameAntidiag a b

danger :: Candidate -> Stack -> Bool
danger candidate = any (danger' candidate)
...
prettyPrint2 n queens = concat . map row $ [1 .. n]
    where row i = map cell [1 .. n] ++ "\n"
            where cell j
                    | elem (i, j) queens = 'Q'
                    | danger (i, j) queens = '#'
...
fixFirst n s
    | snd f > n = Nothing
    | danger f (tail s) = fixFirst n (nextCol f : tail s)
    | otherwise = Just s
    where f = head s
...
finish n s = if length newS > n then tail newS else finish n newS
    where newS = step n s
