...
instance Functor TwoList where
  ...
  fmap f (TwoNode x y tail) = TwoNode (f x) (f y) (fmap f tail)
...
count a = getSum . foldMap (\e -> if e == a then Sum 1 else Sum 0)
...
inBoth f = concatMap (\e -> if elem e f then [e] else [])
...
instance Foldable List where
  foldr _ b Empty = b
  foldr f b (LNode head tail) = f head (foldr f b tail)
...
instance Foldable TwoList where
  foldr _ b TwoEmpty = b
  foldr f b (TwoNode x y tail) = f x . f y . foldr f b $ tail
