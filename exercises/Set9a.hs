...
sumSuccess = sumOrNoData . rights
  where
    sumOrNoData xs = if length xs == 0 then Left "no data" else Right (sum xs)
    rights = foldr prependRight []
    prependRight (Left _) = id
    prependRight (Right x) = (x :)
...
open tryCode (Locked code) = if tryCode == code then Open code else Locked code
open _ l = l
...
instance Eq Text where
  Text a == Text b = removeWS a == removeWS b
...
compose f g = [(x, z) | (x, y) <- f, let maybeZ = lookup y g, isJust maybeZ, let (Just z) = maybeZ]
...
permute p xs = map ((xs !!) . fromJust . ($ p) . elemIndex) [0 .. length p - 1]
