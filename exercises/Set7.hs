...
member x (Set l) = elem x l

-- add a member to a set
addOrdered :: Ord a => a -> [a] -> [a]
addOrdered val [] = [val]
addOrdered val (x : xs)
  | val == x = x : xs
  | val < x = val : x : xs
  | otherwise = x : addOrdered val xs

add :: Ord a => a -> Set a -> Set a
add val (Set l) = Set (addOrdered val l)
...
data State = Start | Error | Finished
  | Eggs | EggsAndFlour | EggsAndSugar
  | AllIngredients | Mixed
  deriving (Eq,Show)
...
step EggsAndFlour AddSugar = AllIngredients
step EggsAndSugar AddFlour = AllIngredients
...
reverseNonEmpty (h :| t) = head r :| tail r
  where r = reverse (h : t)
...
passwordAllowed password (ContainsSome chars) = any (\c -> elem c chars) password
passwordAllowed password (DoesNotContain chars) = all (\c -> not (elem c chars)) password
passwordAllowed password (And a b) = passwordAllowed password a && passwordAllowed password b
passwordAllowed password (Or a b) = passwordAllowed password a || passwordAllowed password b
