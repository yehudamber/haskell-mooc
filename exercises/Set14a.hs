...
greetText name = T.concat [T.pack "Hello, ", shortened, T.pack "!"]
    where shortened =
            if T.length name <= 15
                then name
                else T.concat [T.take 15 name, T.pack "..."]
...
shout :: T.Text -> T.Text
shout = T.unwords
    . zipWith (\n w -> if even n then w else T.toUpper w) [1..]
    . T.words
...
longestRepeat = maximum . (0 :) . map T.length . T.groupBy (==)
...
byteRange b = if B.null b then 0 else B.maximum b - B.minimum b
...
pingpong b = BL.concat . cycle . map BL.fromStrict $ [b, B.reverse b]
