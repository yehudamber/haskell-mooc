...
longest :: Ord a => [[a]] -> [a]
longest (l:lists) = foldr (\list current ->
    if length list > length current || (length list == length current && head list < head current)
        then list
        else current) l lists
...
incrementKey key = map (\(k, v) -> (k, if k == key then v + 1 else v))
...
freqs = foldr (\x -> Map.alter inc x) Map.empty
    where
        inc Nothing = Just 1
        inc (Just n) = Just (n + 1)
...
transfer from to amount bank =
    if isValid
        then Map.insert from newFromAmount . Map.insert to newToAmount $ bank
        else bank
    where
        fromAmount = Map.lookup from bank
        toAmount = Map.lookup to bank
        newFromAmount = maybe 0 (subtract amount) fromAmount
        newToAmount = maybe 0 (+ amount) toAmount
        isValid = amount >= 0 && fromAmount /= Nothing && toAmount /= Nothing && newFromAmount >= 0
...
maxIndex arr = maximumBy (comparing (arr !)) (indices arr)
