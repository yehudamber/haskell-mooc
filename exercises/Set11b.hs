...
hSelectLines' :: [Int] -> [String] -> [String]
hSelectLines' [] _ = []
hSelectLines' (i : is) strs = strs !! (i - 1) : hSelectLines' is strs

hSelectLines :: Handle -> [Int] -> IO [String]
hSelectLines h nums = do
    lines <- hFetchLines h
    return (hSelectLines' nums lines)
...
interact' f state = do
    line <- getLine
    let (continue, message, newState) = f (line, state)
    putStrLn message
    if continue
