...
split s = elemIndex ' ' s ?> \i -> let (a, b) = splitAt i s in Just (a, tail b)

checkStrings :: (String -> Bool) -> (String, String) -> Maybe (String, String)
checkStrings f (a, b) = if f a && f b then Just (a, b) else Nothing
...
checkNumber = checkStrings (not . any isDigit)
...
checkCapitals = checkStrings (isUpper . head)
...
safeIndex xs i = if i < 0 || i >= length xs then Nothing else Just (xs !! i)
...
countAndLog pred (x : xs) = if pred x then msg (show x) >> rest >>= (return . (+ 1)) else rest
  where rest = countAndLog pred xs
...
count x = do
  pairs <- get
  let n = case lookup x pairs of
            (Just n) -> n
            Nothing -> 0
  put ((x, n + 1) : filter (\(y, _) -> y /= x) pairs)
