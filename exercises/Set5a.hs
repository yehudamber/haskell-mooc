...
toList Empty = []
toList (Pair key val rest) = (key, val) : toList rest

fromList :: [(k,v)] -> KeyVals k v
fromList = foldr (\(key, val) rest -> Pair key val rest) Empty
...
toNat :: Int -> Maybe Nat
toNat z = if z < 0 then Nothing else Just . maybe Zero PlusOne . toNat $ z - 1
...
toBin :: Int -> Bin
toBin x = (if x `mod` 2 == 0 then O else I) . toBin' $ x `div` 2
